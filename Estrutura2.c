...
public:
    ...
    void inserir(int elemento) {
        if (!estaCheia()) {
            ...
            std::cout << "A pilha está cheia. Não é possível inserir mais elementos.\n";
        }
    }
    ...
    void remover() {
        if (!estaVazia()) {
            ...
            std::cout << "A pilha está vazia. Não há elementos para remover.\n";
        }
    }
...
int main() {
    int tamanhoMaximo;
    std::cout << "Digite o tamanho máximo da pilha: ";
    ...
    std::cout << "A pilha está vazia? " << (pilha.estaVazia() ? "Sim" : "Não") << "\n";
    std::cout << "A pilha está cheia? " << (pilha.estaCheia() ? "Sim" : "Não") << "\n";

    pilha.inserir(42);  // Inserir um elemento
    std::cout << "Elementos da pilha após inserção: ";
    pilha.mostrarPilha();

    pilha.remover();  // Remover um elemento
    std::cout << "Elementos da pilha após remoção: ";
    pilha.mostrarPilha();
